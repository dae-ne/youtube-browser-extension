...
export default class RemoveAdblockErrorMessageFeature extends Feature {
    /**
     * This mutation observer is used to watch for changes in the error screen. When the error
     * screen appears, it will click the video to try to reload its source. It also removes the
    ...
    private readonly errorScreenObserver = new MutationObserver(mutations => {
        mutations.forEach(mutation => {
            ...
            this.videoSrcRemovedObserver.observe(video, {
                attributes: true,
                attributeFilter: ['src']
            });
        });
     ...
     * miniplayer) is currently opened. Can disconnect the observers for the video pages if the
     ...
     *
     * @remarks
     * For some reason, the behavior of the error screen is different when the playlist is opened,
